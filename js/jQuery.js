...
jQuery.extend( {
	...
	isNumeric function( obj ) {
		 ...
		 parseFloat NaNs numeric-cast false positives (nulltruefalse)
	...
	type function( obj ) {
		if ( obj == null ) {
		 ...
		 Support Android4.0, iOS6 (functionish RegExp)
...
var i,
	...
	slice = arr.slice,
	 ...
	 httpjsperf.comthor-indexof-vs-for5
	...
	booleans = checkedselectedasyncautofocusautoplaycontrolsdeferdisabledhiddenismaploopmultipleopenreadonlyrequiredscoped,
	...
	whitespace = [x20trnf],
	 ...
	 httpwww.w3.orgTRCSS21syndata.html#value-def-identifier
	...
	matchExpr = {
		...
		CHILD new RegExp( ^(onlyfirstlastnthnth-last)-(childof-type)(( + whitespace +
			(evenodd(([+-])(d)n) + whitespace + (([+-]) + whitespace +
			(d+))) + whitespace + )), i ),
		bool new RegExp( ^( + booleans + )$, i ),
		 For use in libraries implementing .is()
		 We use this for POS matching in `select`
		needsContext new RegExp( ^ + whitespace + [+~](evenoddeqgtltnthfirstlast)(( +
	...
	rescape = 'g,
	 ...
	 CSS escapes httpwww.w3.orgTRCSS21syndata.html#escaped-characters
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	rbuggyMatches = [];
	 ...
	 See httpbugs.jquery.comticket13378
	rbuggyQSA = [];
	...
	if ( (support.qsa = rnative.test( document.querySelectorAll )) ) {
		...
		assert(function( div ) {
			 ...
			 httpbugs.jquery.comticket12359
			docElem.appendChild( div ).innerHTML = a id=' + expando + 'a +
			 ...
			 httpmsdn.microsoft.comen-uslibraryiehh465388.aspx#attribute_section
...
support.sortDetached = assert(function( div1 ) {
 ...
 httpmsdn.microsoft.comen-uslibraryms536429%28VS.85%29.aspx
...
Data.prototype = {
	...
	remove function( owner, key ) {
		...
		if ( key === undefined  jQuery.isEmptyObject( cache ) ) {
			 ...
			 httpscode.google.compchromiumissuesdetailid=378607
			if ( owner.nodeType ) {
...
var rscriptType = ( ^$(javaecma)scripti );
...
var
	rkeyEvent = ^key,
	rmouseEvent = ^(mousepointercontextmenudragdrop)click,
	rtypenamespace = ^([^.])(.(.+));
...
jQuery.event = {
	...
	add function( elem, types, handler, data, selector ) {
		...
		var handleObjIn, eventHandle, tmp,
		 ...
		 Don't attach events to noData or textcomment nodes (but allow plain objects)
		if ( !elemData ) {
			return;
		}
	...
	mouseHooks {
		...
		filter function( event, original ) {
			var eventDoc, doc, body,
			 ...
			 Calculate pageXY if missing and clientXY available
	...
	special {
		...
		focus {
			 ...
			 Fire native event if possible so blurfocus sequence is correct
			trigger function() {
...
jQuery.Event = function( src, props ) {
 ...
 httpwww.w3.orgTR2003WD-DOM-Level-3-Events-20030331ecma-script-binding.html
jQuery.Event.prototype = {
 ...
 httpscode.google.compchromiumissuesdetailid=470258
...
var
	rxhtmlTag = (!areabrcolembedhrimginputlinkmetaparam)(([w-]+)[^])gi,
...
jQuery.extend( {
	...
	clone function( elem, dataAndEvents, deepDataAndEvents ) {
		...
		if ( !support.noCloneChecked && ( elem.nodeType === 1  elem.nodeType === 11 ) &&
			 ...
			 We eschew Sizzle here for performance reasons httpjsperf.comgetall-vs-sizzle2
...
( function() {
	...
	container.style.cssText = border0;width8px;height0;top0;left-9999px; +
		padding0;margin-top1px;positionabsolute;
	...
	function computeStyleTests() {
		div.style.cssText =
			 ...
			 Support Firefox29, Android 2.3
			 Vendor-prefix box-sizing
			-webkit-box-sizingborder-box;-moz-box-sizingborder-box;box-sizingborder-box; +
	...
	jQuery.extend( support, {
		...
		reliableMarginRight function() {
			...
			marginDiv.style.cssText = div.style.cssText =
				 ...
				 Support Android 2.3
				 Vendor-prefix box-sizing
				-webkit-box-sizingcontent-box;box-sizingcontent-box; +
				displayblock;margin0;border0;padding0;
...
function curCSS( elem, name, computed ) {
	...
	if ( computed ) {
		 ...
		 httpdev.w3.orgcsswgcssom#resolved-values
...
function getWidthOrHeight( elem, name, extra ) {
	...
	if ( document.msFullscreenElement && window.top !== window ) {
	 ...
	 Some non-html elements return undefined for offsetWidth, so check for nullundefined
	 svg - httpsbugzilla.mozilla.orgshow_bug.cgiid=649285
	 MathML - httpsbugzilla.mozilla.orgshow_bug.cgiid=491668
	if ( val = 0  val == null ) {
	 ...
	 Use the active box-sizing model to addsubtract irrelevant styles
	return ( val +
...
function defaultPrefilter( elem, props, opts ) {
	...
	if ( elem.nodeType === 1 && ( height in props  width in props ) ) {
		...
		opts.overflow = [ style.overflow, style.overflowX, style.overflowY ];
		 ...
		 Set display property to inline-block for heightwidth
		 animations on inline elements that are having widthheight animated
...
jQuery.fx.speeds = {
 ...
 httpweb.archive.orgweb20100324014747httpblindsignals.comindex.php200907jquery-delay
	...
	propHooks {
		tabIndex {
			get function( elem ) {
				 ...
				 httpfluidproject.orgblog20080109getting-setting-and-removing-tabindex-values-with-javascript
...
support.focusin = onfocusin in window;
 ...
 Related ticket - httpscode.google.compchromiumissuesdetailid=449857
if ( !support.focusin ) {
	...
	ajax function( url, options ) {
		...
		if ( s.crossDomain == null ) {
			urlAnchor = document.createElement( a );
			 ...
			 Support IE8-11+
			 IE throws exception if url is malformed, e.g. httpexample.com80x
			try {
...
jQuery.fn.extend( {
	...
	position function() {
		...
		var offsetParent, offset,
		 ...
		 Fixed elements are offset from window (parentOffset = {top0, left 0},
		...
		jQuery.fn[ funcName ] = function( margin, value ) {
			...
			return access( this, function( elem, type, value ) {
				...
				if ( jQuery.isWindow( elem ) ) {
					 ...
					 As of 582012 this will yield incorrect results for Mobile Safari, but there
					 isn't a whole lot we can do. See pull request at this URL for discussion
					 httpsgithub.comjqueryjquerypull764
...
jQuery.noConflict = function( deep ) {
 ...
 (#7102#comment10, httpsgithub.comjqueryjquerypull557)
